...
void MainWindow::onCreatePolynomial() {
    ...
    currentPoly = p;
    lblHighest->setText("Highest: " + (currentPoly.coeff.empty() ? QString("-") : QString::number(currentPoly.coeff.back().re) + (currentPoly.coeff.back().im>=0?"+":"") + QString::number(currentPoly.coeff.back().im) + "i"));
...
void MainWindow::onSend(){
    ...
    QString op = "evaluate";
    QJsonObject msg; msg["op"]=op; msg["polynom"]=polynomToJson(currentPoly);
